...
{
    public class LeftStation : IWork
    {
        //1上2下 3上4下
        readonly Action<List<ImageDisplayViewModel>, string, List<string>> PushDisResult;
        ...
        public LeftStation(Action<List<ImageDisplayViewModel>, string, List<string>> action, CancellationTokenSource _token, int num, int _startIndex)
        {
            Token = _token;
        ...
        public Task InspectTask()
        {
            return Task.Run(() =>
            {
                ...
                while (!Token.IsCancellationRequested)
                {
                    ...
                    {
                        Log.AppLog(ConstHelper.Left + $"等待触发拍照.....{temp}");
                        while (pos > ConstHelper.PhotoNum || pos <= 0)
                        {
                            pos = Common.GetValue<int>(ConstHelper.LeftCamera);
                            Thread.Sleep(20);
                            if (!Common.PlcContent || !Common.IsStart)
                            {
                                Log.ErrorLog($"检测到{(Common.PlcContent ? "自动状态停止" : "PLC断线")},左工位检测程序退出");
                                if (TProduct != null)
                                {
                        ...
                        Log.AppLog(ConstHelper.Left + $"上次触发指令{temp}，此次触发指令{pos}");
                        if (!Common.PlcContent || !Common.IsStart)
                            continue;
                        if (pos == 1 || TProduct == null) //当拍照次数为1时清除上次的结果 清除算法任务
                        {
                            ...
                            Inspect.Clear();
                            TProduct = new Product(CameraNumber, ConstHelper.Left);
                        }
                        //收到拍照命令信号,回应下位机
                        Common.Write(ConstHelper.LeftCamera, 0);
                        for (int i = 0; i < CameraNumber; i++)
                        {
                            ...
                            PhotoGraph[index] = Task.Run(() =>
                            {
                                ...
                                if (photoResult == null)
                                    Log.ErrorLog(ConstHelper.Left + $" 第{theps + 1}次拍照时第{index + StartIndex + 1}相机取图失败");
                        ...
                        //相机拍照完成，允许PLC运动
                        Common.Write(ConstHelper.LeftCameraFinish, pos);
                        //算法任务
                        //当前产品拍照次数  
                        var thepos = pos - 1;
                        Inspect.Add(Task.Run(() =>
                        {
                            for (int i = 0; i < CameraNumber; i++)
                            {
                                ...
                                {
                                    str = "Bottom: ";
                                    List<OpenCvSharp.Point> pix = Common.CutArray(ConstHelper.Left, thepos * ConstHelper.ProductNumber + (index == 1 ? 0 : ConstHelper.MaterialNum / 2));
                        ...
                        if (pos >= 2 && pos <= ConstHelper.PhotoNum)
                        {
                            ...
                            if (flag)
                            {
                                ...
                                PushDisResult?.Invoke(bitmaps, ConstHelper.Left, ConstHelper.IntToStringArray(TempResult, pos - 1));
                            }
                        }
                        #endregion
                        ...
                        if (pos == ConstHelper.PhotoNum)
                        {
                            ...
                            Common.Write(ConstHelper.LeftResult, TProduct.ResultNum);
                            List<ImageDisplayViewModel> bitmaps = new List<ImageDisplayViewModel>();
                            for (int i = 0; i < CameraNumber; i++)
                            {
                                //缩放结果图便于展示
                            ...
                            PushDisResult?.Invoke(bitmaps, ConstHelper.Left, ConstHelper.IntToStringArray(TProduct.ResultNum, pos));
                    ...
                    //        if (pos == i && File.Exists(Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + "\\" + i.ToString() + ".jpg"))
                    //        {
                    //            FileStream fs = new FileStream(Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + "\\" + i.ToString() + ".jpg", FileMode.Open);
                    ...
                    //        {
                    //            bitmaps.Add(new ImageDisplayViewModel() { DisImage = item.ToBitmapSource().Clone() });
                    //            item.Dispose();
                    //        }
                    //    });
                    //    Random random = new Random();
                    //    PushDisResult?.Invoke(bitmaps, ConstHelper.Left, ConstHelper.IntToStringArray(random.Next(1000, 16777218), (pos + 1) * 2 * ConstHelper.ProductNumber));
                    //    pos++;
                    //    if (pos >= 4)
                    //        pos = 0;
                    //    Thread.Sleep(50);
